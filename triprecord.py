...
output_path_foryellow = raw_data_path_pre + 'prepared_data/Yellow.csv'
output_path_forgreen = raw_data_path_pre + 'prepared_data/Green.csv'
output_path_forfhv = raw_data_path_pre + 'prepared_data/FHV.csv'
output_path_forfhvhv = raw_data_path_pre + 'prepared_data/FHVHV.csv'

output_path_forall = raw_data_path_pre+ 'prepared_data/trip_all.csv'
...
def count_5minutes_forgreen(subfile_path, input_region_geo_Manhanttan_path):
    ...
    df_green['lpep_pickup_datetime_min_5'] = df_green['lpep_pickup_datetime'].apply(lambda x: prepare_for_5minutes(x).strftime('%Y-%m-%d %H:%M'))
    df_green['lpep_dropoff_datetime_min_5'] = df_green['lpep_dropoff_datetime'].apply(lambda x: prepare_for_5minutes(x).strftime('%Y-%m-%d %H:%M'))
    
    df_green = df_green[(df_green['PULocationID'] != '') & (df_green['DOLocationID'] != '')]
    df_green = df_green[(df_green['lpep_pickup_datetime_year'] == '2020') | (df_green['lpep_pickup_datetime_year'] == '2019') | (df_green['lpep_dropoff_datetime_year'] == '2020') | (df_green['lpep_dropoff_datetime_year'] == '2019')]
    ...
    df_green = df_green[(df_green['PULocationID'].isin(Manhattan_region_id)) & (df_green['DOLocationID'].isin(Manhattan_region_id))]

    df_green_pickup = df_green.groupby(['lpep_pickup_datetime_min_5', 'PULocationID']).count()['lpep_pickup_datetime']
    df_green_dropoff = df_green.groupby(['lpep_dropoff_datetime_min_5', 'DOLocationID']).count()['lpep_dropoff_datetime']
    Con_df_green = pd.concat([df_green_pickup, df_green_dropoff], axis=1)
    Con_df_green = Con_df_green.reset_index()
    Con_df_green.columns = ['datetime_min_5', 'region_id', 'Pickup_number', 'Dropoff_number']
    Con_df_green = Con_df_green.fillna(0)
    return Con_df_green
...
def count_5minutes_foryellow(subfile_path, input_region_geo_Manhanttan_path):
    ...
    df_yellow['lpep_pickup_datetime_min_5'] = df_yellow['lpep_pickup_datetime'].apply(lambda x: prepare_for_5minutes(x).strftime('%Y-%m-%d %H:%M'))
    df_yellow['lpep_dropoff_datetime_min_5'] = df_yellow['lpep_dropoff_datetime'].apply(lambda x: prepare_for_5minutes(x).strftime('%Y-%m-%d %H:%M'))
    
    df_yellow = df_yellow[(df_yellow['PULocationID'] != '') & (df_yellow['DOLocationID'] != '')]
    df_yellow = df_yellow[(df_yellow['lpep_pickup_datetime_year'] == '2020') | (df_yellow['lpep_pickup_datetime_year'] == '2019') | (df_yellow['lpep_dropoff_datetime_year'] == '2020') | (df_yellow['lpep_dropoff_datetime_year'] == '2019')]
    ...
    df_yellow_pickup = df_yellow.groupby(['lpep_pickup_datetime_min_5', 'PULocationID']).count()['lpep_pickup_datetime']
    df_yellow_dropoff = df_yellow.groupby(['lpep_dropoff_datetime_min_5', 'DOLocationID']).count()['lpep_dropoff_datetime']
    Con_df_yellow = pd.concat([df_yellow_pickup, df_yellow_dropoff], axis=1)
    Con_df_yellow = Con_df_yellow.reset_index()
    Con_df_yellow.columns = ['datetime_min_5', 'region_id', 'Pickup_number', 'Dropoff_number']
    
    Con_df_yellow = Con_df_yellow.fillna(0)
...
def count_5minutes_forfhv(subfile_path, input_region_geo_Manhanttan_path):
    ...
    df_fhv['lpep_pickup_datetime_min_5'] = df_fhv['lpep_pickup_datetime'].apply(lambda x: prepare_for_5minutes(x).strftime('%Y-%m-%d %H:%M'))
    df_fhv['lpep_dropoff_datetime_min_5'] = df_fhv['lpep_dropoff_datetime'].apply(lambda x: prepare_for_5minutes(x).strftime('%Y-%m-%d %H:%M'))
    
    df_fhv = df_fhv[(df_fhv['PULocationID'] != '') & (df_fhv['DOLocationID'] != '')]
    df_fhv = df_fhv[(df_fhv['lpep_pickup_datetime_year'] == '2020') | (df_fhv['lpep_pickup_datetime_year'] == '2019') | (df_fhv['lpep_dropoff_datetime_year'] == '2020') | (df_fhv['lpep_dropoff_datetime_year'] == '2019')]
    
    # classify region_id in manhattan
    Manhattan_region_id = []
    Manhattan_region_id = select_manhatten_regionid(input_region_geo_Manhanttan_path)
    df_fhv = df_fhv.loc[(df_fhv['PULocationID'].isin(Manhattan_region_id)) & (df_fhv['DOLocationID'].isin(Manhattan_region_id))]
    
    df_fhv_pickup = df_fhv.groupby(['lpep_pickup_datetime_min_5', 'PULocationID']).count()['lpep_pickup_datetime_year']
    df_fhv_dropoff = df_fhv.groupby(['lpep_dropoff_datetime_min_5', 'DOLocationID']).count()['lpep_pickup_datetime_year']
    Con_df_fhv = pd.concat([df_fhv_pickup, df_fhv_dropoff], axis=1)
    Con_df_fhv = Con_df_fhv.reset_index()
    Con_df_fhv.columns = ['datetime_min_5', 'region_id', 'Pickup_number', 'Dropoff_number']
    
    Con_df_fhv = Con_df_fhv.fillna(0)
    return Con_df_fhv
...
def count_5minutes_forfhvhv(subfile_path, input_region_geo_Manhanttan_path):
    ...
    df_fhvhv['lpep_pickup_datetime_min_5'] = df_fhvhv['lpep_pickup_datetime'].apply(lambda x: prepare_for_5minutes(x).strftime('%Y-%m-%d %H:%M'))
    df_fhvhv['lpep_dropoff_datetime_min_5'] = df_fhvhv['lpep_dropoff_datetime'].apply(lambda x: prepare_for_5minutes(x).strftime('%Y-%m-%d %H:%M'))
    
    df_fhvhv = df_fhvhv[(df_fhvhv['PULocationID'] != '') & (df_fhvhv['DOLocationID'] != '')]
    df_fhvhv = df_fhvhv[(df_fhvhv['lpep_pickup_datetime_year'] == '2020') | (df_fhvhv['lpep_pickup_datetime_year'] == '2019') | (df_fhvhv['lpep_dropoff_datetime_year'] == '2020') | (df_fhvhv['lpep_dropoff_datetime_year'] == '2019')]
    
    # classify region_id in manhattan
    Manhattan_region_id = []
    Manhattan_region_id = select_manhatten_regionid(input_region_geo_Manhanttan_path)
    df_fhvhv = df_fhvhv.loc[(df_fhvhv['PULocationID'].isin(Manhattan_region_id)) & (df_fhvhv['DOLocationID'].isin(Manhattan_region_id))]
    
    df_fhvhv_pickup = df_fhvhv.groupby(['lpep_pickup_datetime_min_5', 'PULocationID']).count()['lpep_pickup_datetime_year']
    df_fhvhv_dropoff = df_fhvhv.groupby(['lpep_dropoff_datetime_min_5', 'DOLocationID']).count()['lpep_pickup_datetime_year']
    Con_df_fhvhv = pd.concat([df_fhvhv_pickup, df_fhvhv_dropoff], axis=1)
    Con_df_fhvhv = Con_df_fhvhv.reset_index()
    Con_df_fhvhv.columns = ['datetime_min_5', 'region_id', 'Pickup_number', 'Dropoff_number']
    
    Con_df_fhvhv = Con_df_fhvhv.fillna(0)
    return Con_df_fhvhv
